function f = errorbar3(x,y,er,direction,color,varargin)
...
% modified MR: Dec 2016, takes an additional parameter "transparency" of
% the errorbar as last added argument, if none is given the bar is not
% transparent.
...
% check number of optional arguments (e.g. transparency)
numvarargs = length(varargin);
if numvarargs > 1
    error('TooManyInputs requires at most 1 optional input (transparency)')
end
%
optargs = { 1.0 }; % by default, no transparency.
optargs(1:numvarargs) = varargin; %overwrite defaults by given optional inputs
[ transparency ] = optargs{:};
